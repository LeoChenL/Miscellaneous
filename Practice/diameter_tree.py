...
def diameter_of_binary_tree(root):
    if root == None:
        return 0
    dia = tree_height(root.left) + tree_height(root.right) + 2
    # print("left", tree_height(root.left))
    # print("right", tree_height(root.right))
    return dia
...
def tree_height(rt):
    ...
    h = max(tree_height(rt.left), tree_height(rt.right)) + 1
    return h
